    ...
    <div className="space-y-10">
      <section className="glass-panel px-6 py-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.32em] text-neutral-400">Protocol Operations</p>
            <h1 className="mt-2 text-2xl font-semibold text-neutral-900">Study Orchestration & EDC Command</h1>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-neutral-200 bg-white px-4 py-2 text-xs font-medium text-neutral-600 transition hover:border-brand-300 hover:text-brand-600"
            >
      ...
      <section className="glass-panel px-6 py-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.32em] text-neutral-400">Studies</p>
            <h2 className="mt-2 text-xl font-semibold text-neutral-900">Portfolio Overview</h2>
          </div>
          <span className="text-xs text-neutral-500">{studiesQuery.isFetching ? "Updating…" : `${studies.length} total`}</span>
        </div>
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full divide-y divide-neutral-200 text-left text-sm text-neutral-700">
            <thead>
              <tr>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-400">Study</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-400">Phase</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-400">Status</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-400">Sites</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-400">Subjects</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-400">Updated</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-400" aria-label="actions" />
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-100">
              {studies.map((study) => (
                <tr key={study.id} className="hover:bg-brand-50/60">
                  <td className="px-4 py-3">
                    <div className="font-semibold text-neutral-900">{study.name}</div>
                    <div className="text-xs uppercase tracking-[0.22em] text-neutral-400">{study.code}</div>
                  </td>
                  <td className="px-4 py-3 text-neutral-500">{study.phase ?? "—"}</td>
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${
                        statusColors[study.status] ?? "bg-neutral-100 text-neutral-600"
                      }`}
                    >
                      {study.status}
                    </span>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-neutral-500">{study.sites?.length ?? 0}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-neutral-600">
                      {study.activeSubjects ?? 0}
                      <span className="text-neutral-400"> / {study.totalSubjects ?? 0}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-neutral-500">{new Date(study.updatedAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-right">
                    <button
                      type="button"
                      onClick={() => router.push(`/studies/${study.id}`)}
                      className="inline-flex items-center gap-1 rounded-full border border-neutral-200 px-3 py-1 text-xs font-semibold text-neutral-600 transition hover:border-brand-300 hover:text-brand-600"
                    >
                      Open
      ...
      {showCreate && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-neutral-900/60 px-4">
          <div className="w-full max-w-xl rounded-3xl border border-neutral-200 bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-semibold text-neutral-900">Create Study</h3>
            <p className="mt-1 text-sm text-neutral-500">Define the protocol shell, you can refine details later.</p>
            <div className="mt-5 grid gap-4">
              <div className="grid gap-2">
                <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">Code *</label>
                <input
                  value={draft.code}
                  onChange={(event) => setDraft((prev) => ({ ...prev, code: event.target.value }))}
                  placeholder="SYN-001"
                  className="rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-2 text-sm text-neutral-700 focus:border-brand-300 focus:outline-none"
                />
              </div>
              <div className="grid gap-2">
                <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">Name *</label>
                <input
                  value={draft.name}
              ...
              <div className="grid gap-2">
                <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">Phase</label>
                <select
                  value={draft.phase}
                ...
                >
                  ...
                  {phases.map((phase) => (
                    <option key={phase} value={phase} className="bg-white text-neutral-900">
                      {phase}
                    </option>
                  ))}
                </select>
              ...
              <div className="grid gap-2">
                <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">Therapeutic Area</label>
                <input
              ...
              <div className="grid gap-2">
                <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">Sponsor</label>
                <input
                  value={draft.sponsor}
              ...
              <div className="grid gap-2 md:grid-cols-2 md:gap-4">
                <div className="grid gap-2">
                  <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">Start</label>
                  <input
                    type="date"
                ...
                <div className="grid gap-2">
                  <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">End</label>
                  <input
                    type="date"
                    value={draft.endDate ?? ""}
              ...
              <div className="grid gap-2">
                <label className="text-xs uppercase tracking-[0.32em] text-neutral-400">Synopsis</label>
                <textarea
                  value={draft.summary}
                  onChange={(event) => setDraft((prev) => ({ ...prev, summary: event.target.value }))}
                  placeholder="Primary objective, comparator, endpoints..."
                  className="h-32 w-full rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-700 focus:border-brand-300 focus:outline-none"
                />
              </div>
            </div>
            <div className="mt-6 flex items-center justify-between">
              {createStudy.status === "success" && (
                <span className="inline-flex items-center gap-2 text-xs font-medium text-emerald-600">
                  <CheckCircleIcon className="h-4 w-4 text-emerald-500" /> Study created
                </span>
              )}
              {createStudy.status === "error" && (
                <span className="text-xs text-rose-600">{createStudy.error?.message ?? "Failed to create study"}</span>
              )}
              <div className="ml-auto flex gap-3">
                <button
                  ...
                  className="rounded-full border border-neutral-200 px-4 py-2 text-xs font-semibold text-neutral-600 transition hover:border-brand-300 hover:text-brand-600"
                >
                  Cancel
                </button>
                <button
