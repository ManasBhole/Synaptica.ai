...
export default function StudyDetailPage() {
  ...
  const statusDisplay = study ? statusTone[study.status] ?? "bg-white/10 text-white" : "bg-white/10 text-white";

  if (!studyId) {
    return <div className="glass-panel px-6 py-6 text-white/70">Missing study id.</div>;
  }

  if (!study && studyQuery.isLoading) {
    return (
      <div className="glass-panel px-6 py-12 text-center text-white/60">
        Loading study...
      </div>
    );
  }
  ...
  const tabButton = (key: string, label: string, icon: JSX.Element) => (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold transition ${
        activeTab === key ? "bg-gradient-to-r from-brand-500/30 via-brand-500/10 to-transparent text-white" : "text-white/60 hover:bg-white/10"
      }`}
    >
      {icon}
      {label}
    ...
    <div className="space-y-10">
      <section className="glass-panel px-6 py-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.32em] text-white/40">Protocol {study.code}</p>
            <h1 className="mt-2 text-2xl font-semibold text-white">{study.name}</h1>
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs text-white/50">
              {study.phase && <span className="rounded-full bg-white/5 px-3 py-1 text-white/70">Phase: {study.phase}</span>}
              {study.therapeuticArea && <span className="rounded-full bg-white/5 px-3 py-1 text-white/70">Area: {study.therapeuticArea}</span>}
              {study.sponsor && <span className="rounded-full bg-white/5 px-3 py-1 text-white/70">Sponsor: {study.sponsor}</span>}
              <span className="rounded-full bg-white/5 px-3 py-1 text-white/70">Sites: {sites.length}</span>
              <span className="rounded-full bg-white/5 px-3 py-1 text-white/70">Subjects: {study.totalSubjects ?? 0}</span>
            </div>
          </div>
          <div className="flex flex-col items-end gap-4">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold text-white/70 transition hover:border-white/20 hover:text-white"
            >
        ...
        <section className="glass-panel px-6 py-6">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-white/70">
              <p className="text-xs uppercase tracking-[0.32em] text-white/40">Sites</p>
              <p className="mt-2 text-2xl font-semibold text-white">{sites.length}</p>
              <p className="mt-1 text-xs text-white/50">Across geographies</p>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-white/70">
              <p className="text-xs uppercase tracking-[0.32em] text-white/40">Active Subjects</p>
              <p className="mt-2 text-2xl font-semibold text-white">{study.activeSubjects ?? 0}</p>
              <p className="mt-1 text-xs text-white/50">On treatment or follow-up</p>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-white/70">
              <p className="text-xs uppercase tracking-[0.32em] text-white/40">Forms</p>
              <p className="mt-2 text-2xl font-semibold text-white">{forms.length}</p>
              <p className="mt-1 text-xs text-white/50">eCRFs available</p>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-white/70">
              <p className="text-xs uppercase tracking-[0.32em] text-white/40">Visit templates</p>
              <p className="mt-2 text-2xl font-semibold text-white">{visits.length}</p>
              <p className="mt-1 text-xs text-white/50">Scheduled touchpoints</p>
            </div>
          </div>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-white/70">
              <h3 className="text-lg font-semibold text-white">Recent Audit Trail</h3>
              <ul className="mt-4 space-y-3 text-xs text-white/60">
                {auditLogs.slice(0, 6).map((log) => (
                  <li key={log.id} className="flex flex-col rounded-2xl border border-white/10 bg-black/20 px-3 py-3">
          ...
          <div className="flex flex-col gap-6 lg:flex-row">
            <div className="flex-1">
              ...
              <div className="mt-4 space-y-3">
                {sites.map((site) => (
                  <div key={site.id} className="rounded-3xl border border-white/10 bg-white/5 px-4 py-4 text-sm text-white/70">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-white font-semibold">{site.name}</p>
                        <p className="text-xs uppercase tracking-[0.22em] text-white/40">{site.siteCode}</p>
                      </div>
                      <span className="text-xs text-white/50">{new Date(site.createdAt).toLocaleDateString()}</span>
                    </div>
                    <p className="mt-2 text-white/60">Principal Investigator: {site.principalInvestigator ?? "—"}</p>
                    <p className="text-white/60">Country: {site.country ?? "—"}</p>
                    {site.contact?.email && <p className="text-white/50">Contact: {String(site.contact.email)}</p>}
                  </div>
                ))}
                {sites.length === 0 && <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-6 text-center text-white/50">No sites yet</div>}
              </div>
            </div>
            <div className="w-full max-w-sm rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-white/70">
              <h4 className="text-base font-semibold text-white">Add site</h4>
              <div className="mt-4 space-y-3">
                <input
                  value={siteDraft.siteCode}
                  onChange={(event) => setSiteDraft((prev) => ({ ...prev, siteCode: event.target.value }))}
                  placeholder="SITE-001"
                  className="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-2 text-white focus:border-brand-400 focus:outline-none"
                />
                <input
                ...
                {createSite.status === "success" && <p className="text-xs text-emerald-200">Site added</p>}
              </div>
            </div>
          </div>
        </section>
      ...
      {activeTab === "forms" && (
        <section className="glass-panel px-6 py-6">
          <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            <div>
              <h3 className="text-lg font-semibold text-white">Form library</h3>
              <div className="mt-4 space-y-3">
                {forms.map((form) => (
                  <div key={form.id} className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-sm text-white/70">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-white font-semibold">{form.name}</p>
                        <p className="text-xs uppercase tracking-[0.22em] text-white/40">{form.slug}</p>
                      </div>
                      <span className="text-xs text-white/50">v{form.version}</span>
                    </div>
                    <p className="mt-2 text-white/60">{form.description ?? "No description"}</p>
                    <p className="mt-2 text-xs text-white/40">Fields: {Array.isArray((form.schema as any)?.fields) ? (form.schema as any).fields.length : 0}</p>
                  </div>
                ))}
                {forms.length === 0 && <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-6 text-center text-white/50">No forms yet</div>}
              </div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-white/70">
              <h4 className="text-base font-semibold text-white">Compose form</h4>
              <div className="mt-4 space-y-3">
                <input
                ...
                <textarea
                  ...
                  className="h-24 w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-white focus:border-brand-400 focus:outline-none"
                />
                <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
                  <div className="flex items-center justify-between text-xs text-white/60">
                    <span>Fields</span>
                    <button type="button" onClick={handleAddField} className="inline-flex items-center gap-1 text-brand-200">
                  ...
                  <div className="mt-3 space-y-2">
                    {formDraft.fields.map((field, index) => (
                      <div key={index} className="grid gap-2 rounded-2xl border border-white/10 bg-black/30 p-3">
                        <div className="grid gap-2 md:grid-cols-2">
                          <input
                            ...
                            className="rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-white focus:border-brand-400 focus:outline-none"
                          />
                          <input
                            value={field.label}
                            onChange={(event) => setFormDraft((prev) => {
                        ...
                        <div className="grid gap-2 md:grid-cols-2">
                          ...
                          <label className="inline-flex items-center gap-2 text-xs text-white/50">
                            <input
                              ...
                              className="h-4 w-4 rounded border-white/20 bg-black/30 text-brand-400 focus:ring-brand-400"
                            />
                            Required
                          </label>
                        </div>
                ...
                {createForm.status === "success" && <p className="text-xs text-emerald-200">Form created</p>}
              </div>
            </div>
          </div>
        </section>
      ...
      {activeTab === "visits" && (
        <section className="glass-panel px-6 py-6">
          <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            <div>
              <h3 className="text-lg font-semibold text-white">Visit schedule</h3>
              <div className="mt-4 space-y-3">
                {visits.map((visit) => (
                  <div key={visit.id} className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-sm text-white/70">
                    <div className="flex items-center justify-between">
                      <div className="text-white font-semibold">{visit.name}</div>
                      <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-white/70">Day {visit.visitOrder}</span>
                    </div>
                    <div className="mt-2 text-white/60">Window: {visit.windowStartDays ?? 0} to {visit.windowEndDays ?? 0} days</div>
                    <div className="text-white/60">Required: {visit.required ? "Yes" : "Optional"}</div>
                    <div className="text-white/50">Forms: {visit.forms?.length ? visit.forms.join(", ") : "—"}</div>
                  </div>
                ))}
                {visits.length === 0 && <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-6 text-center text-white/50">No visit templates yet</div>}
              </div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-white/70">
              <h4 className="text-base font-semibold text-white">Add visit</h4>
              <div className="mt-4 space-y-3">
                <input
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    placeholder="-2"
                    className="rounded-2xl border border-white/10 bg-black/30 px-4 py-2 text-white focus:border-brand-400 focus:outline-none"
                  />
                  <input
                ...
                <label className="text-xs text-white/50">
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-2 text-white focus:border-brand-400 focus:outline-none"
                  />
                </label>
                <button
                  type="button"
                ...
                {createVisit.status === "success" && <p className="text-xs text-emerald-200">Visit created</p>}
              </div>
            </div>
          </div>
        </section>
      ...
      {activeTab === "subjects" && (
        <section className="glass-panel px-6 py-6">
          <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            <div>
              ...
              <div className="mt-4 overflow-x-auto">
                <table className="min-w-full divide-y divide-white/10 text-left text-sm text-white/70">
                  <thead>
                    <tr>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/50">Subject</th>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/50">Site</th>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/50">Status</th>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/50">Randomization</th>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/50">Consented</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/10">
                    {subjects.map((subject) => (
                      <tr key={subject.id} className="hover:bg-white/5">
                        <td className="px-4 py-3 font-semibold text-white">{subject.subjectCode}</td>
                        <td className="px-4 py-3 text-white/60">{sites.find((site) => site.id === subject.siteId)?.name ?? "—"}</td>
                        <td className="px-4 py-3 text-white/60">{subject.status}</td>
                        <td className="px-4 py-3 text-white/60">{subject.randomizationArm ?? "—"}</td>
                        <td className="px-4 py-3 text-white/60">{subject.consentedAt ? new Date(subject.consentedAt).toLocaleDateString() : "Pending"}</td>
                      </tr>
                    ))}
                    {subjects.length === 0 && (
                      <tr>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-white/70">
              <h4 className="text-base font-semibold text-white">Enroll subject</h4>
              <div className="mt-4 space-y-3">
                <input
                ...
                {enrollSubjectMutation.status === "success" && <p className="text-xs text-emerald-200">Subject enrolled</p>}
              </div>
            </div>
          </div>
        </section>
      ...
      {activeTab === "consents" && (
        <section className="glass-panel px-6 py-6">
          <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            <div>
              <h3 className="text-lg font-semibold text-white">Consent versions</h3>
              <div className="mt-4 space-y-3">
                {consentVersions.map((version) => (
                  <div key={version.id} className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-sm text-white/70">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-white font-semibold">{version.title ?? version.version}</p>
                        <p className="text-xs uppercase tracking-[0.22em] text-white/40">{version.version}</p>
                      </div>
                      <span className="text-xs text-white/50">Effective {new Date(version.effectiveAt).toLocaleDateString()}</span>
                    </div>
                    <p className="mt-2 text-white/60">{version.summary ?? "—"}</p>
                    {version.documentUrl && (
                      <a href={version.documentUrl} target="_blank" rel="noreferrer" className="text-xs text-brand-200">
                        View document
                      </a>
                    )}
                  </div>
                ))}
                {consentVersions.length === 0 && <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-6 text-center text-white/50">No consent versions yet</div>}
              </div>
            </div>
            <div className="space-y-6">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-white/70">
                <h4 className="text-base font-semibold text-white">New consent version</h4>
                <div className="mt-4 space-y-3">
                  <input
                  ...
                  {createConsentVersionMutation.status === "success" && <p className="text-xs text-emerald-200">Consent version created</p>}
                </div>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-white/70">
                <h4 className="text-base font-semibold text-white">Record consent</h4>
                <div className="mt-4 space-y-3">
                  <select
                  ...
                  {recordConsentMutation.status === "success" && <p className="text-xs text-emerald-200">Consent recorded</p>}
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === "audit" && (
        <section className="glass-panel px-6 py-6">
          ...
          <div className="mt-4 space-y-3">
            {auditLogs.map((log) => (
              <div key={log.id} className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-sm text-white/70">
                <div className="flex items-center justify-between text-white">
                  <span className="font-semibold">{log.action}</span>
                  <span className="text-xs text-white/50">{new Date(log.createdAt).toLocaleString()}</span>
                </div>
                <p className="text-xs text-white/50">Actor: {log.actor}{log.role ? ` (${log.role})` : ""}</p>
                {log.entity && <p className="text-xs text-white/50">Entity: {log.entity} · {log.entityId}</p>}
                {log.payload && (
                  <pre className="mt-2 max-h-40 overflow-auto rounded-2xl bg-black/30 p-3 text-xs text-white/60">
                    {JSON.stringify(log.payload, null, 2)}
                  </pre>
                )}
              </div>
            ))}
            {auditLogs.length === 0 && <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-6 text-center text-white/50">No audit entries yet</div>}
          </div>
        </section>
      )}
    </div>
